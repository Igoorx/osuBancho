...
namespace osuBancho.Hosts.HTTP
{
                    ...
                    switch (context.Request.Url.AbsolutePath)
                    {
                        case "/":
                            ...
                            try
                            {
                                ...
                                {
                                    MemoryStream receivedStream = new MemoryStream(4096);
                                    await context.Request.InputStream.CopyToAsync(receivedStream);
                                    receivedStream.Seek(0, SeekOrigin.Begin);
                                    
                                    Player player; string playerToken;
                                    if (!string.IsNullOrEmpty(playerToken = context.Request.Headers.Get("osu-token")) &&
                                        receivedStream.Length >= 7 &&
                                        (player = PlayerManager.GetPlayerBySessionToken(playerToken)) != null)
                                    {
                                        await Task.Run(() => player.HandleRawPacket(receivedStream));
                                        player.SerializeCommands(outStream);
                                    } else
